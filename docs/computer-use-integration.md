# Computer use & browser use in NexBot

Decision doc, 2026-08-12. How bots in NexBot get local computer use and
...
│     one TCC prompt, named NexBot          │ unix socket (private)
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is NexBot's, users see ONE
   prompt named NexBot, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `NexBot.app/Contents/Resources/cua-driver`,
...
{ "mcpServers": { "computer": {
    ...
    "env": { "CUA_DRIVER_EMBEDDED": "1", "CUA_DRIVER_HOST_BUNDLE_ID": "com.nexbot.app" }
} } }
```
