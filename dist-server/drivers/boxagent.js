...
export const BoxAgentDriver = {
    ...
    async create(input) {
        ...
        const sendTurn = async (turn) => {
            ...
            if (!token)
                throw new Error('box not configured — add {"box":{"token":"…"}} to ~/.nexbot/config.json');
            if (!boxId)
        ...
        const snapshot = async () => {
            if (!token) {
                return { state: "unavailable", reason: 'no Box token — add {"box":{"token":"…"}} to ~/.nexbot/config.json' };
            }
            try {
                await api("/me");
