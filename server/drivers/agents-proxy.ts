...
//   NEXBOT_TURN_DEPTH   this turn's comms depth (the harness refuses recursion)
...
const DEPTH = Number(process.env.NEXBOT_TURN_DEPTH ?? "0") || 0;

const TOOLS = [
  {
    name: "list_bots",
    description:
      "List the other bots (agents) in this NexBot workspace you can message, with their model and whether they're busy. Call this before ask_bot to discover who's available. There is only one Chief of Staff; never create a second.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "ask_bot",
    description:
      "Send a message to another bot in this workspace and BLOCK until it finishes (or times out). The target's reply text is returned as this tool result so you can summarize in the same turn. If the target is still working, you get a still-working note — do not end the turn silently. Never create a second Chief of Staff. Fight X / challenge X means critique X's existing output with a specialist — never ask_bot X to write the critique of itself.",
    inputSchema: {
      type: "object",
      properties: {
