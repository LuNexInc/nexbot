...
const DENY_TIMEOUT_NOTE =
  "NexBot: nobody answered this permission request in time. Skip this action and finish what you can without it.";
const QUESTION_TIMEOUT_NOTE = "NexBot: nobody answered in time. Use your best judgment and continue.";
  ...
  return {
    ...
    close() {
      for (const p of [...pending.values()]) {
        if (p.ask.kind === "question") p.finish("answer", "NexBot: the turn is ending — wrap up.", "shutdown");
        else p.finish("deny", "NexBot: the turn ended", "shutdown");
      }
      try {
        server.close();
      } catch {}
  ...
  async create(input: DriverCreateInput<ClaudeConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      if (turn.integrations?.computer) {
        mcpServers.computer = {
          ...
          env: {
            ...NODE_ENV_FLAG,
            NEXBOT_BOX_ID: turn.integrations.computer.boxId,
            NEXBOT_BOX_TOKEN: turn.integrations.computer.token,
          },
        };
    ...
    const snapshot = async (): Promise<ProviderSnapshot> => {
      const version = await new Promise<string | null>((resolve) => {
        execFileCli(config.cli, ["--version"], { timeout: 8000, env: { ...process.env, PATH: augmentedPath() } }, (err, stdout) =>
