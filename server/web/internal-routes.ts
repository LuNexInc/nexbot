...
export async function handleInternalRoutes(args: RouteArgs): Promise<boolean> {
  ...
  if (method === "POST" && path === "/api/internal/send-bot") {
    ...
    // The delegator's turn is ending after announcing the handoff. Record a
    // promise so the harness resolves it back into the delegator's thread with
    // the result when the target completes — instead of trusting a generic
    // "[Teammate update]" to be the promised ping-back.
    recordHandoffPromise({ fromBotId: from.id, fromThreadId: from.threadId, toBotId: target.id, request: message });
