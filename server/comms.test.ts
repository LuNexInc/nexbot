...
describe("mentionedBots", () => {
  ...
  it("matches a tag at a word start, case-insensitively", () => {
    expect(mentionedBots("hey @nexus, look", peers).map((b) => b.id)).toEqual(["3"]);
    expect(mentionedBots("@Nexus first thing", peers).map((b) => b.id)).toEqual(["3"]);
  });
  ...
  it("dedupes repeats and collects multiple bots", () => {
    expect(mentionedBots("@Nexus and @New Bot and @Nexus", peers).map((b) => b.id)).toEqual(["3", "1"]);
  });
...
posixOnly("comms e2e (fake ACP fleet)", () => {
  ...
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "nexbot-comms-test-"));
