...
describe("augmentedPath", () => {
  afterEach(() => {
    delete process.env.NEXBOT_EXTRA_PATH;
  ...
  it("keeps the existing PATH entries first", () => {
    ...
    // NEXBOT_EXTRA_PATH is unset here, so the inherited PATH leads
    expect(path.split(delimiter)[0]).toBe(firstExisting);
  });

  it("prepends NEXBOT_EXTRA_PATH and dedupes", () => {
    process.env.NEXBOT_EXTRA_PATH = ["/tmp/nexbot-extra", "/tmp/nexbot-extra"].join(delimiter);
    resetPathCacheForTests();
    const parts = augmentedPath().split(delimiter);
    expect(parts[0]).toBe("/tmp/nexbot-extra");
    expect(parts.filter((p) => p === "/tmp/nexbot-extra")).toHaveLength(1);
  });
  ...
  posixIt("makes a CLI in a known install dir spawnable despite a bare PATH", async () => {
    ...
    const fake = join(bin, "nexbot-fake-cli");
