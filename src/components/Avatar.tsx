...
// Exact LuNex mascot silhouette from the website's
    ...
    case "friendly":
      return (
        <>
          ...
          <g className="-mouth"><path d="M88 105 Q101 118 114 105" {...line} /></g>
        </>
      );
    case "focused":
      return (
        <>
          ...
          <g className="-mouth"><path d="M90 111 Q101 107 112 111" {...line} /></g>
        </>
      );
    case "thinking":
      return (
        <>
          ...
          <g className="-mouth"><path d="M93 111 Q101 105 110 109" {...line} /></g>
        </>
      );
    case "excited":
      return (
        <>
          ...
          <g className="-mouth"><path d="M88 102 Q101 124 114 102 Q101 109 88 102 Z" fill={INK} /></g>
        </>
      );
    case "sleepy":
      return (
        <>
          ...
          <g className="-mouth"><ellipse cx="101" cy="110" rx="6" ry="8" fill={INK} /></g>
        </>
      );
    case "surprised":
      return (
        <>
          ...
          <g className="-mouth"><circle cx="101" cy="111" r="8" fill={INK} /></g>
        </>
      );
    case "skeptical":
      return (
        <>
          ...
          <g className="-mouth"><path d="M91 112 Q101 107 112 113" {...line} /></g>
        </>
      );
    case "worried":
      return (
        <>
          ...
          <g className="-mouth"><path d="M89 115 Q101 103 113 115" {...line} /></g>
        </>
      );
    case "mischievous":
      return (
        <>
          ...
          <g className="-mouth"><path d="M89 106 Q103 118 116 103 Q103 110 89 106 Z" fill={INK} /></g>
        </>
      );
    case "deadpan":
      return (
        <>
          ...
          <g className="-mouth"><path d="M88 110 L114 110" {...line} /></g>
        </>
      );
  }
}
...
function trackEyes(event: ReactPointerEvent<SVGSVGElement>) {
  ...
  event.currentTarget.style.setProperty("---eye-x", `${Math.max(-1, Math.min(1, x)) * 3.4}px`);
  event.currentTarget.style.setProperty("---eye-y", `${Math.max(-1, Math.min(1, y)) * 2.6}px`);
}
...
function AvatarComponent({
  ...
  return (
    <svg
      ...
      style={{ "---color": fill, "---eye-x": "0px", "---eye-y": "0px" } as CSSProperties}
    >
      <defs>
        <clipPath id={clipId}>
          <path d={BODY} />
      ...
      <g key={motionKey} className={`-motion -motion--${motion}`}>
        {hasAlert && (
          <g className="-fx -fx--alert" aria-hidden="true">
        ...
        {hasEllipsis && (
          <g className="-fx -fx--ellipsis" aria-hidden="true">
            <circle className="-ellipsis-dot -ellipsis-dot--one" cx="72" cy="101" r="8" fill={fill} />
            <circle className="-ellipsis-dot -ellipsis-dot--two" cx="100" cy="101" r="10" fill={fill} />
            <circle className="-ellipsis-dot -ellipsis-dot--three" cx="130" cy="101" r="8" fill={fill} />
          </g>
        )}

        {hasRibbons && (
          <g className="-fx -fx--ribbons" aria-hidden="true">
            <path className="-ribbon -ribbon--one" pathLength="1" d="M55 119 C79 137 119 140 150 116" stroke={fill} />
            <path className="-ribbon -ribbon--two" pathLength="1" d="M54 112 C85 126 119 125 145 103" stroke="#fcfcfc" />
            <path className="-ribbon -ribbon--three" pathLength="1" d="M62 126 C87 145 126 145 156 121" stroke={shade(fill, 44)} />
          </g>
        )}

        {hasBurst && (
          <g className="-fx -fx--burst" aria-hidden="true">
            <circle className="-burst-dot -burst-dot--one" cx="100" cy="100" r="5" fill={fill} />
            <circle className="-burst-dot -burst-dot--two" cx="100" cy="100" r="4" fill="#fcfcfc" />
            <circle className="-burst-dot -burst-dot--three" cx="100" cy="100" r="6" fill={shade(fill, 42)} />
            <circle className="-burst-dot -burst-dot--four" cx="100" cy="100" r="3.5" fill={fill} />
            <path className="-burst-ray -burst-ray--one" d="M100 100 L100 73" stroke={fill} />
            <path className="-burst-ray -burst-ray--two" d="M100 100 L125 88" stroke="#fcfcfc" />
            <path className="-burst-ray -burst-ray--three" d="M100 100 L82 121" stroke={shade(fill, 42)} />
          </g>
        )}

        {hasOrbit && (
          <g className="-orbit" aria-hidden="true">
            <path className="-speed-line -speed-line--one" pathLength="1" d="M38 119 C23 81 45 42 78 29" />
            <path className="-speed-line -speed-line--two" pathLength="1" d="M55 151 C32 126 29 94 39 70" />
            <circle className="-orbit-dot -orbit-dot--one" cx="47" cy="54" r="4.6" fill={fill} />
            <circle className="-orbit-dot -orbit-dot--two" cx="151" cy="65" r="3.4" fill="#fcfcfc" />
            <circle className="-orbit-dot -orbit-dot--three" cx="155" cy="132" r="5.2" fill={fill} />
          </g>
        )}

        <g className="-character">
          <g className="-solid" transform="rotate(-20 100 100)">
              <path className="-body" d={BODY} fill={`url(#${surfaceId})`} />
              <path
                className="-bevel"
                d={BODY}
                fill="none"
