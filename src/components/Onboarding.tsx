...
export function Onboarding({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    if (step === 2 && isElectron) {
      const poll = () => window.?.permStatus?.().then(setPerms).catch(() => {});
      poll();
      ...
      <div className="flex w-[460px] flex-col rounded-2xl border border-hairline/40 bg-panel p-8">
        {step === 0 && (
          <div className="flex flex-col items-center">
            <Avatar color="green" expression="friendly" size={72} />
            <h1 className="mt-4 text-[20px] font-semibold text-ink">Welcome to NexBot</h1>
            <p className="mt-1.5 text-center text-[14px] leading-relaxed text-ink-secondary">
              Bots that do real work on their own computer. Optional name and
            ...
            <div className="mt-4 flex flex-col gap-2.5">
              <div className="flex items-center justify-between gap-3 rounded-xl bg-card p-3.5">
                ...
                {perms?.mic === "granted" ? (
                  ...
                  <button
                    onClick={() => window.?.permOpenSettings?.("mic")}
                  ...
                  <button
                    onClick={() =>
                      window.?.permRequestMic?.().then(() => window.?.permStatus?.().then(setPerms))
                    }
                    className="shrink-0 rounded-lg bg-raised px-3 py-1.5 text-[13px] text-ink hover:bg-raised-hover"
                  >
                    Enable
